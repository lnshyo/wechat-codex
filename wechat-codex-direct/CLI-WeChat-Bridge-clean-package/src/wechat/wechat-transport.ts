...
function parseWechatApiEnvelope(raw: string): WechatApiEnvelope | null {
  try {
...
export function assertWechatApiResponseOk(raw: string, endpoint: string): string {
  const envelope = parseWechatApiEnvelope(raw);
  if (!envelope) {
    return raw;
  }

  const hasRetError =
    typeof envelope.ret === "number" && envelope.ret !== 0;
  const hasErrcodeError =
    typeof envelope.errcode === "number" && envelope.errcode !== 0;

  if (!hasRetError && !hasErrcodeError) {
    return raw;
  }

  throw new Error(
    `${endpoint} failed: ret=${envelope.ret ?? "(none)"} errcode=${envelope.errcode ?? "(none)"} errmsg=${envelope.errmsg ?? ""}`,
  );
}

export function isWechatContextTokenRejectedError(error: unknown): boolean {
  return (
    error instanceof Error &&
    /sendmessage failed: ret=-2\b/i.test(error.message)
  );
}
...
export class WeChatTransport {
  ...
  private async sendMessage(
    ...
    try {
      ...
      assertWechatApiResponseOk(raw, "sendmessage");
    } catch (error) {
      if (isWechatContextTokenRejectedError(error)) {
        this.clearContextToken(recipientId);
        const message =
          error instanceof Error ? error.message : String(error);
        throw new Error(
          `${message}. Cleared cached context token for ${recipientId}; ask the user to send a new WeChat message to refresh context.`,
  ...
  private clearContextToken(senderId: string): void {
    if (!this.contextTokenCache.delete(senderId)) {
      return;
    }

    if (this.contextTokenCache.size === 0) {
