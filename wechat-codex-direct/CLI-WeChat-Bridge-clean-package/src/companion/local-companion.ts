...
export function parseCliArgs(argv: string[]): LocalCompanionCliOptions {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (arg === "--help" || arg === "-h") {
      process.stdout.write(
        [
          "Usage: local-companion --adapter <codex|claude|opencode> [--cwd <path>] [--headless]",
          "",
    ...
    void (async () => {
      try {
        switch (message.payload.command) {
          case "send_input":
            await adapter.sendInput(message.payload.text, message.payload.options);
            sendResponse(message.id, true);
            break;
          case "set_pinned_session":
            await adapter.setPinnedSession?.(
