...
const ANSI_ESCAPE_RE =
  ...
  /\x1b(?:[@-Z\\-_]|\[[0-?]*[ -/]*[@-~])/g;
const CONTROL_COMMAND_INVISIBLE_RE = /[\u200B-\u200D\u2060\uFEFF]/g;
const CONTROL_COMMAND_UNICODE_SPACE_RE = /[\u00A0\u1680\u180E\u2000-\u200A\u202F\u205F\u3000]/g;

export type SystemCommand =
  | { type: "status" }
  | { type: "probe_01" }
  | { type: "route_independent" }
  | { type: "route_bound" }
...
export type LocalCodexControlCommand = { type: "bind_thread" };
...
const WECHAT_REPLY_STYLE_PROMPT_PREFIX = [
  "[WeChat bridge note]",
  "Reply to the user in Simplified Chinese unless the user explicitly requests another language.",
  "For each user message, provide one consolidated final reply instead of multiple follow-up replies.",
].join("\n");

const WECHAT_ATTACHMENT_PROMPT_GUIDANCE = [
...
export function parseSystemCommand(text: string): SystemCommand | null {
  const trimmed = normalizeControlCommandText(text);
  ...
  switch (command) {
    ...
    case "/new":
    case "/2":
      return { type: "route_independent" };
    case "/bound":
    case "/1":
      return { type: "route_bound" };
    case "/resume":
...
export function parseLocalCodexControlCommand(
  text: string,
): LocalCodexControlCommand | null {
  const normalized = normalizeControlCommandText(text);
  if (!normalized) {
    return null;
  }
...
export function parseWechatControlCommand(
  ...
  const normalizedRaw = normalizeControlCommandText(text);
  if (normalizedRaw === "01") {
    return { type: "probe_01" };
  }
...
function normalizeControlCommandText(text: string): string {
  return normalizeOutput(text)
    .normalize("NFKC")
    .replace(CONTROL_COMMAND_INVISIBLE_RE, "")
    .replace(CONTROL_COMMAND_UNICODE_SPACE_RE, " ")
    .trim();
}
...
export function buildWechatInboundPrompt(text: string): string {
  ...
  const sections = [WECHAT_REPLY_STYLE_PROMPT_PREFIX];
  if (
    shouldInjectWechatAttachmentPrompt(normalized) &&
    !normalized.includes("```wechat-attachments")
  ) {
    sections.push(WECHAT_ATTACHMENT_PROMPT_GUIDANCE);
  }
...
export function parseWechatFinalReply(text: string): ParsedWechatFinalReply {
  ...
  for (const line of lines) {
    ...
    const rawKind = parsed[1];
    const rawAttachmentPath = parsed[2];
    if (!rawKind || !rawAttachmentPath) {
      return extractInlineWechatAttachments(normalized);
    }
    const kind = rawKind as WechatAttachmentKind;
    const attachmentPath = resolveWechatAttachmentPath(rawAttachmentPath);
    if (!attachmentPath) {
      return extractInlineWechatAttachments(normalized);
    }
...
export function formatStatusReport(
  ...
  const boundSessionId =
    bridgeState.boundSessionId ?? bridgeState.boundThreadId;
  ...
  return [
    ...
    `bound_session_id: ${boundSessionId ?? "(none)"}`,
    `route_mode: ${bridgeState.routeMode ?? "(none)"}`,
    `route_independent_once: ${bridgeState.routeIndependentOnce ?? false}`,
...
export function formatApprovalMessage(
  ...
  if (isClaude) {
    return [
      "Claude 需要权限确认。",
      ...(pending.toolName ? [`工具：${pending.toolName}`] : []),
      ...(pending.detailPreview
        ? [`${pending.detailLabel ?? "详情"}：${pending.detailPreview}`]
        : pending.commandPreview
          ? [`详情：${pending.commandPreview}`]
          : []),
      "回复 /confirm、confirm 或 yes 继续。",
      "回复 /deny、deny 或 no 拒绝。",
    ].join("\n");
  }

  return [
    `${pending.source === "shell" ? "Shell" : "CLI"} 需要审批。`,
    `适配器：${adapterState.kind}`,
    `代码：${pending.code}`,
    `摘要：${pending.summary}`,
    `目标：${pending.commandPreview}`,
    "回复 /confirm <code> 继续，或回复 /deny 拒绝。",
  ].join("\n");
}
...
export function formatPendingApprovalReminder(
  ...
  if (adapterState.kind === "claude") {
    const target = pending.toolName
      ...
      : pending.commandPreview;
    return `当前有待处理的 Claude 权限请求：${truncatePreview(target, 140)}。回复 /confirm、confirm 或 yes 继续，或回复 /deny、deny 或 no 拒绝。`;
  }

  return `当前有待处理的审批：${pending.commandPreview}。回复 /confirm ${pending.code} 或 /deny。`;
}
