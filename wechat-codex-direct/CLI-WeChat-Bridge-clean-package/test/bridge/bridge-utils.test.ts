...
describe("parseSystemCommand", () => {
  test("parses supported control commands", () => {
    ...
    expect(parseSystemCommand("/new")).toEqual({ type: "route_independent" });
    expect(parseSystemCommand("/new=")).toEqual({ type: "route_independent" });
    expect(parseSystemCommand("/2")).toEqual({ type: "route_independent" });
    expect(parseSystemCommand("/bound")).toEqual({ type: "route_bound" });
    expect(parseSystemCommand("/bound=")).toEqual({ type: "route_bound" });
    expect(parseSystemCommand("/1")).toEqual({ type: "route_bound" });
  ...
  test("normalizes full-width punctuation and invisible characters before parsing", () => {
    expect(parseSystemCommand(" \u200b／２\u200b ")).toEqual({
      type: "route_independent",
    });
    expect(parseSystemCommand(" \u200b／１\u200b ")).toEqual({
      type: "route_bound",
    });
  });
...
describe("parseLocalCodexControlCommand", () => {
  test("recognizes bind aliases from the local codex side", () => {
    expect(parseLocalCodexControlCommand("绑定微信")).toEqual({
      type: "bind_thread",
    });
    expect(parseLocalCodexControlCommand("/bind")).toEqual({
      type: "bind_thread",
    });
    expect(parseLocalCodexControlCommand("/BIND=")).toEqual({
      type: "bind_thread",
    });
    expect(parseLocalCodexControlCommand(" \u200b／ＢＩＮＤ＝\u200b ")).toEqual({
      type: "bind_thread",
    });
  });

  test("ignores unrelated local codex text", () => {
    expect(parseLocalCodexControlCommand("继续分析这个文件")).toBeNull();
    expect(parseLocalCodexControlCommand("/new")).toBeNull();
  });
});
...
describe("parseWechatControlCommand", () => {
  ...
  test("recognizes the plain 01 smoke-test probe before forwarding to Codex", () => {
    expect(
      parseWechatControlCommand("01", {
        adapter: "codex",
        hasPendingConfirmation: true,
      }),
    ).toEqual({ type: "probe_01" });
    expect(
      parseWechatControlCommand(" 01 ", {
        adapter: "claude",
        hasPendingConfirmation: false,
...
describe("formatStatusReport", () => {
  test("includes bridge routing details for bound versus temporary sessions", () => {
    const report = formatStatusReport(
      {
        ...
        sharedSessionId: "thread-active",
        sharedThreadId: "thread-active",
    ...
    expect(report).toContain("route_mode: independent");
    expect(report).toContain("route_independent_once: true");
    expect(report).toContain("bound_session_id: thread-bound");
  });
});
...
describe("wechat inbound prompt injection", () => {
  test("injects attachment guidance for explicit send-to-WeChat requests", () => {
    ...
    expect(prompt).toContain("Reply to the user in Simplified Chinese");
  ...
  test("skips prompt injection for ordinary non-send requests and existing protocol blocks", () => {
    ...
    expect(buildWechatInboundPrompt(explicitProtocol)).toContain(
      "Reply to the user in Simplified Chinese",
    );
  });
});
...
describe("adapter-aware message formatting", () => {
  ...
  test("formats final reply and failure messages by adapter", () => {
    ...
    expect(formatTaskFailedMessage("claude", "Boom")).toBe("Claude 任务失败：\nBoom");
  });
  ...
  test("formats Claude approval prompts without a required code", () => {
    ...
    expect(formatApprovalMessage(pending, claudeAdapterState)).toContain("工具：Bash");
    expect(formatApprovalMessage(pending, claudeAdapterState)).toContain("command：npm test");
    expect(formatApprovalMessage(pending, claudeAdapterState)).not.toContain("代码：");
    expect(formatApprovalMessage(pending, claudeAdapterState)).toContain(
      "/confirm、confirm 或 yes",
    );
    expect(formatPendingApprovalReminder(pending, claudeAdapterState)).toContain(
      "Bash (npm test)",
    );
    expect(formatApprovalMessage(pending, codexAdapterState)).toContain("代码：ABC123");
