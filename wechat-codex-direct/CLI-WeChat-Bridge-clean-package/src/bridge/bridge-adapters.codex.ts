...
export function shouldTrackPinnedCodexThread(params: {
  pinnedThreadId?: string | null;
  candidateThreadId?: string | null;
}): boolean {
  if (!params.pinnedThreadId || !params.candidateThreadId) {
    return true;
  }

  return params.pinnedThreadId === params.candidateThreadId;
}

export function shouldDeferRpcTurnCompletionToSessionLog(params: {
  renderMode?: AdapterOptions["renderMode"];
  turnOrigin: BridgeTurnOrigin;
  status: string;
  finalText: string | null;
}): boolean {
  return (
    params.renderMode === "panel" &&
    params.turnOrigin === "local" &&
...
export class CodexPtyAdapter extends AbstractPtyAdapter {
  ...
  private suppressedReplayThreadId: string | null = null;
  private suppressedReplayUntilMs = 0;
  private pendingPassiveReplaySuppression = false;
  ...
  private sessionReplayScanPending = false;
  ...
  constructor(options: AdapterOptions) {
    super(options);
    this.pendingPassiveReplaySuppression = options.renderMode === "panel";
  ...
  override async start(): Promise<void> {
    ...
    try {
      ...
      } else if (this.isNativePanelMode()) {
  ...
  override async sendInput(
    text: string,
    options: BridgeSendInputOptions = {},
  ): Promise<void> {
    if (this.isNativePanelMode()) {
      await this.sendPanelTurn(text, options);
      return;
    }

    if (options.freshSession) {
      throw new Error(
        'Fresh Codex threads require the visible "wechat-codex" panel companion.',
      );
    }
  ...
  async setPinnedSession(sessionId: string | null): Promise<void> {
    const nextPinnedThreadId =
      typeof sessionId === "string" && sessionId.trim()
        ? sessionId.trim()
        : null;
    this.pinnedThreadId = nextPinnedThreadId;

    if (this.pendingThreadFollowId && this.pendingThreadFollowId !== nextPinnedThreadId) {
    ...
    if (
      ...
      !shouldTrackPinnedCodexThread({
        pinnedThreadId: nextPinnedThreadId,
        candidateThreadId: this.pendingThreadAnnouncement.threadId,
      })
    ) {
  ...
  private isHeadlessPanelMode(): boolean {
    return this.isNativePanelMode() && this.options.headless === true;
  }

  private isCodexClientRunning(): boolean {
    if (this.isHeadlessPanelMode()) {
      return Boolean(this.appServer && this.rpcSocket);
    }

    return this.isNativePanelMode() ? Boolean(this.nativeProcess) : Boolean(this.pty);
  }

  private isPanelTransportReady(): boolean {
    if (!this.isNativePanelMode()) {
      return false;
    }
    ...
    return Boolean(this.nativeProcess);
  }
  ...
  private stopSessionPolling(): void {
    ...
    this.sessionReplayScanPending = false;
  ...
  private async pollSessionLog(): Promise<void> {
    ...
    if (this.sessionReplayScanPending && this.sessionIgnoreBeforeMs !== null) {
      this.sessionIgnoreBeforeMs = null;
      this.sessionReplayScanPending = false;
    }
  }
  ...
  private maybeApplyRecentSessionFallback(): void {
    ...
    if (
      !shouldTrackPinnedCodexThread({
        pinnedThreadId: this.pinnedThreadId,
    ...
    if (this.sessionFilePath !== candidate.filePath) {
      ...
      this.sessionPartialLine = "";
      this.sessionFinalText = null;
      this.sessionReplayScanPending = this.sessionIgnoreBeforeMs !== null;
    }
  }
  ...
  private async sendPanelTurn(
    ...
    if (!this.isPanelTransportReady()) {
      throw new Error("codex panel is not running.");
    }
    this.recoverCompletedWechatTurnAfterFinalReplyBeforeNextInput();
    ...
    const threadId = options.freshSession
  ...
  private attachRpcSocket(socket: WebSocket): void {
    ...
    this.pendingPassiveReplaySuppression = this.isNativePanelMode();
  ...
  private async restoreInitialSharedThreadIfNeeded(): Promise<void> {
    if (
      !this.resumeThreadId ||
      (this.isNativePanelMode() && !this.isHeadlessPanelMode())
    ) {
      return;
    }
  ...
  private async ensureThreadStarted(): Promise<string> {
    ...
    return await this.startFreshThread();
  }
  ...
  private async startFreshThread(
    ...
    this.pendingThreadFollowId = null;
    this.updateSharedThread(threadId, options);
    return threadId;
  }
  ...
  private async resumeSharedThread(
    ...
    this.sessionFinalText = null;
    this.pendingThreadFollowId = null;
    this.pendingPassiveReplaySuppression = false;
    this.armReplaySuppression(resumedThreadId);
  ...
  private recoverCompletedWechatTurnAfterFinalReplyBeforeNextInput(): void {
    const activeTurn = this.activeTurn;
    if (!activeTurn) {
      return;
    }

    if (this.hasCompletedTurn(activeTurn.turnId)) {
      this.clearFinalReplyCompletionTimerForTurn(activeTurn.turnId);
      this.clearPendingApprovalState();
      this.setActiveTurn(null);
      this.cleanupTurnArtifacts(activeTurn.turnId);
      if (this.state.status === "busy" || this.state.status === "awaiting_approval") {
        this.setStatus("idle", "Recovered stale completed Codex turn.");
      }
      return;
    }

    const hasPendingApproval = Boolean(this.pendingApproval || this.pendingApprovalRequest);
    const finalText = this.collectTurnOutput(activeTurn.turnId);
    if (
      !shouldRecoverCodexWechatTurnAfterFinalReplyBeforeNextInput({
    ...
    this.cleanupTurnArtifacts(activeTurn.turnId);
    if (this.state.status !== "stopped") {
      this.setStatus("idle", "Recovered delayed Codex completion before the next WeChat message.");
    }
    this.emit({
      type: "task_complete",
    ...
    this.rememberCompletedTurn(activeTurn.turnId);
  }
  ...
  private updateSharedThread(
    ...
    if (!threadId || threadId !== this.suppressedReplayThreadId) {
      this.clearReplaySuppression();
    }
    if (!threadId) {
      this.clearPendingThreadAnnouncement();
      this.announcedThreadId = null;
    ...
    if (previousThreadId !== threadId) {
      ...
      this.sessionReplayScanPending = threadId !== null;
      this.nextSessionFallbackScanAtMs = 0;
      this.emit({
        type: "status",
        status: this.state.status,
  ...
  private armReplaySuppression(threadId: string): void {
    if (!this.isNativePanelMode()) {
      return;
    }

    this.suppressedReplayThreadId = threadId;
    this.suppressedReplayUntilMs = Date.now() + CODEX_STARTUP_WARMUP_MS;
  }

  private updatePassiveReplaySuppression(
    threadId: string,
    options: {
      reason: BridgeThreadSwitchReason;
    ...
    if (!this.pendingPassiveReplaySuppression) {
      return;
    }

    if (options.reason !== "local_follow" && options.reason !== "local_session_fallback") {
      return;
    }

    this.pendingPassiveReplaySuppression = false;
    this.armReplaySuppression(threadId);
  }

  private clearReplaySuppression(): void {
    this.suppressedReplayThreadId = null;
    this.suppressedReplayUntilMs = 0;
  }

  private shouldSuppressReplayedLocalTurn(threadId: string): boolean {
    if (!this.isNativePanelMode()) {
      return false;
    }

    if (!this.suppressedReplayThreadId || threadId !== this.suppressedReplayThreadId) {
      return false;
    }

    if (Date.now() >= this.suppressedReplayUntilMs) {
      this.clearReplaySuppression();
      return false;
    }
  ...
  private setActiveTurn(activeTurn: CodexActiveTurn | null): void {
    ...
    if (!activeTurn && this.pendingThreadFollowId) {
      ...
      if (
        !shouldTrackPinnedCodexThread({
          pinnedThreadId: this.pinnedThreadId,
          candidateThreadId: pendingThreadId,
        })
      ) {
        return;
      }
  ...
  private trackLocalSharedThread(
    ...
    this.updatePassiveReplaySuppression(threadId, options);
  ...
  private identifyTrackedTurn(
    ...
    if (this.shouldSuppressReplayedLocalTurn(threadId)) {
      return null;
    }
  ...
  private handleTrackedTurnStarted(trackedTurn: CodexActiveTurn): void {
    ...
    if (
      ...
      !shouldTrackPinnedCodexThread({
        pinnedThreadId: this.pinnedThreadId,
        candidateThreadId: trackedTurn.threadId,
      })
    ) {
      return;
    }
  ...
  private handleTurnCompleted(
    ...
    if (
      shouldDeferRpcTurnCompletionToSessionLog({
        renderMode: this.options.renderMode,
        turnOrigin: completedTrackedTurn.origin,
        status,
        finalText,
      })
    ) {
