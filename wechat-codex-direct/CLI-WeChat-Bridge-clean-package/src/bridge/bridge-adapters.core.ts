...
export class LocalCompanionProxyAdapter implements BridgeAdapter {
  ...
  private desiredPinnedSessionId: string | null = null;
  private appliedPinnedSessionId: string | null = null;
  ...
  async sendInput(text: string, options?: BridgeSendInputOptions): Promise<void> {
  ...
  async setPinnedSession(sessionId: string | null): Promise<void> {
    const nextPinnedSessionId =
      typeof sessionId === "string" && sessionId.trim()
        ? sessionId.trim()
        : null;
    this.desiredPinnedSessionId = nextPinnedSessionId;
    if (
      !this.socket ||
      !this.state.pid ||
      this.appliedPinnedSessionId === nextPinnedSessionId
    ) {
      return;
    }

    await this.sendRequest({
      command: "set_pinned_session",
      sessionId: nextPinnedSessionId,
    });
    this.appliedPinnedSessionId = nextPinnedSessionId;
  }
  ...
  private handlePanelSocket(socket: net.Socket): void {
    ...
    const detachListener = attachLocalCompanionMessageListener(socket, (message) => {
      if (!authenticated) {
        ...
        if (this.appliedPinnedSessionId !== this.desiredPinnedSessionId) {
          void this.setPinnedSession(this.desiredPinnedSessionId).catch(() => {
            // Best effort replay after reconnect; a later route sync will retry.
          });
        }
        return;
      }
    ...
    switch (message.type) {
      ...
      case "state":
        ...
        if (
          this.socket &&
          this.state.pid &&
          this.appliedPinnedSessionId !== this.desiredPinnedSessionId
        ) {
          void this.setPinnedSession(this.desiredPinnedSessionId).catch(() => {
            // Best effort replay after the companion reports readiness.
          });
        }
        this.eventSink({
          type: "status",
  ...
  private detachPanelSocket(): void {
    ...
    this.appliedPinnedSessionId = null;
    this.state.pid = undefined;
    this.state.startedAt = undefined;
    this.state.lastInputAt = undefined;
    this.state.lastOutputAt = undefined;
