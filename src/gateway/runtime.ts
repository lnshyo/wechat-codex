...
export function createGatewayRuntime(deps: GatewayRuntimeDeps) {
  ...
  async function finalizeTask(
    ...
    if (result.error) {
      ...
      await sendLocalText(peerUserId, task.contextToken, '\u8bf7\u6c42\u5931\u8d25\uff0c\u8bf7\u7a0d\u540e\u91cd\u8bd5\u3002');
      return;
    }

    if (!result.text) {
      const emptyContentError = 'Codex \u6ca1\u6709\u8fd4\u56de\u4efb\u4f55\u5185\u5bb9\u3002';
  ...
  async function enqueueMessage(input: QueueMessageInput): Promise<void> {
    ...
    if (visibleQueue.length >= maxQueuedTasks) {
      await sendLocalText(
        input.peerUserId,
        input.contextToken,
        `\u4efb\u52a1\u961f\u5217\u5df2\u6ee1\uff08\u6700\u591a ${maxQueuedTasks} \u4e2a\uff09\u3002\u8bf7\u5148\u53d1\u9001 /task \u6216 /stop\u3002`,
      );
      return;
    }
    ...
    if (hasExecutorInFlight) {
      ...
      await sendLocalText(
        input.peerUserId,
        input.contextToken,
        `\u4efb\u52a1 #${task.id} \u5df2\u52a0\u5165\u961f\u5217\uff0c\u5f53\u524d\u4f4d\u7f6e\uff1a${queuePosition}\u3002\u53ef\u53d1\u9001 /task \u67e5\u770b\u961f\u5217\u3002`,
      );
    }
  ...
  return {
    ...
    attachPeerToLatestLocalSession: async (peerUserId: string) => {
      if (!deps.attachPeerToLatestLocalSession) {
        return {
          attached: false,
          error: '\u672c\u5730 Codex \u7a97\u53e3\u540c\u6b65\u672a\u542f\u7528\u3002',
        };
      }

      return deps.attachPeerToLatestLocalSession(peerUserId);
    },
    detachPeerLocalSession: async (peerUserId: string) => {
      if (!deps.detachPeerLocalSession) {
        return false;
      }

      return deps.detachPeerLocalSession(peerUserId);
    },
    enqueueMessage,
    stopPeer,
    resetPeer,
