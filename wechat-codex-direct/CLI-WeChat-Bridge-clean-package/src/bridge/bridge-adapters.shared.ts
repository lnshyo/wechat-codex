...
export function shouldRecoverCodexWechatTurnAfterFinalReplyBeforeNextInput(params: {
...
export function normalizeComparablePath(filePath: string): string {
  ...
  try {
    resolvedPath = fs.realpathSync.native?.(resolvedPath) ?? fs.realpathSync(resolvedPath);
  } catch {
    // Fall back to the unresolved path when the target does not exist yet.
  }
