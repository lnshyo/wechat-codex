...
function normalizeSession(peerUserId: string, session: Partial<Session> | undefined): Session {
  const localSync =
    session?.localSync?.enabled &&
    session.localSync.sessionId &&
    session.localSync.transcriptPath
      ? {
          enabled: true,
          sessionId: session.localSync.sessionId,
          transcriptPath: session.localSync.transcriptPath,
          transcriptCursor: Math.max(0, Math.floor(session.localSync.transcriptCursor ?? 0)),
          transcriptCwd: session.localSync.transcriptCwd,
          lastTranscriptEventAt: session.localSync.lastTranscriptEventAt,
        }
      : undefined;
...
export function createSessionStore() {
  ...
  function list(accountId: string): Session[] {
    const accountDir = join(SESSIONS_DIR, accountId);

    try {
      return readdirSync(accountDir)
        .filter((name) => name.endsWith('.json'))
        .map((name) => name.slice(0, -5))
        .map((peerUserId) => load(accountId, peerUserId));
    } catch {
      return [];
    }
  }

  return { load, save, clear, update, list };
}
